...
function patchIDE() {
    try {
        ...
        try {
            ...
            console.error("[Error in patchIDE - Read package.json]: Không thể đọc hoặc parse package.json của IDE. Chi tiết:", pkgErr.message, pkgErr);
        }
        ...
        sendLogToUI('warning', `Đang tiến hành cấu hình thiết lập kết nối cho IDE (${isESM ? 'ES Module' : 'CommonJS'})...`);

        // Loại bỏ các bản vá cũ hoặc lỗi (bất kỳ dòng nào bắt đầu bằng marker)
        ...
        console.error("[Error in patchIDE]: Lỗi vá IDE tại main.js của Desktop App. Chi tiết lỗi: " + e.message, e);
        return false;
    }
}
